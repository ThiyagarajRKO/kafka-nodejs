(function ($) {
  ...
  FloatingMenu.prototype = {
    init: function (options) {
      ...
      this.menuReference = Math.random();
      this.$this = $(this).data("raw", this);
      this.$menuTemplate = $(this.markup).data(
        "menuReference",
        this.menuReference
      );
      var $li = null;
      ...
      options.items.forEach(function (item) {
        ...
        $a = $("<a href='" + href + "'>" + item.title + "</a>").click(
          { type: type, item: item },
          $.proxy(this.actionHandler, this)
        );
      ...
      $(document).on(
        "click.show." + this.menuReference,
        this.selector,
        $.proxy(this.calloutHandler, this)
      );
    },
    actionHandler: function (event) {
      ...
      switch (event.data.type) {
        case "fn":
          ...
          break;
        case "url":
          event.data.item.blank
            ? window.open(event.data.item.action, "_blank")
            : window.location.assign(event.data.item.action);
          break;
      }
      if (event.data.item.close !== !1) {
        this.hide();
      }
    ...
    show: function (event) {
      ...
      (function (_self) {
        if (!_self.isVisible) {
          _self.$menuTemplate
            .addClass("animated fadeInPosition")
            .appendTo("body");
          _self.attachShowEvents();
        } else {
          _self.$menuTemplate
            .removeClass("animated fadeInPosition")
            .addClass("visible-transit");
        }
    ...
    getRelativeToCalloutPos: function () {
      ...
      switch (this.floatedTo) {
        case "bottom":
          ...
          left =
            $offset.left +
            this.$callout.width() / 2 -
            this.$menuTemplate.width() / 2;
          break;
        case "left":
          top =
            $offset.top +
            this.$callout.height() / 2 -
            this.$menuTemplate.height() / 2;
    ...
    attachShowEvents: function () {
      ...
      $(document).on("click.hide." + this.menuReference, function (event) {
        if (
          !$(event.target).is(_self.selector) &&
          $(event.target).data("menuReference") != _self.menuReference
        ) {
          _self.hide();
        }
      });
